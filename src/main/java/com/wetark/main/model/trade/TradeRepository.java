...
public interface TradeRepository extends BaseRepository<Trade> {
    ...
    @Query("Select trd.event.id as id, trd.event.title as title, max(trd.event.picture) as picture, trd.tradeType as tradeType, MAX(trd.event.createdAt) as createdAt, max(trd.event.expireAt) as expireAt, SUM(trd.price*(trd.initialSize-trd.size)) as totalAmount, SUM(trd.price*trd.size) as totalPendingAmount from Trade trd where trd.user = :user group by trd.event.id, trd.tradeType, trd.event.title")
