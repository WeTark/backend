...
public interface TradeRepository extends BaseRepository<Trade> {
    ...
    @Query("Select trd.event.id as id, trd.tradeType as tradeType, SUM(trd.price*(trd.initialSize-trd.size)) as totalAmount, SUM(trd.price*trd.size) as totalPendingAmount from Trade trd where trd.user = :user group by trd.event.id, trd.tradeType")
